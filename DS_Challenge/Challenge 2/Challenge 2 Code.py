...
class Node:
    ...
    def __repr__(self):
        return str(self.num) + " " + str(self.parent.num if self.parent else None)
...
for i in range(n):
    e = list(map(float, input().split()))
    e = rotate(e)
    point.append(Point([e[0], e[1]], 2))
    point.append(Point([e[2], e[3]], 2))
...
for _ in range(num):
    edges.append([])
for j in range(len(regions_up)):
    x = regions.nodes[regions_up[j]].get_set().num
    y = regions.nodes[regions_down[j]].get_set().num
    if x != y and (x, y) not in edge_dic:
        edge_dic[(x, y)] = 0
        edge_dic[(y, x)] = 0
        x = regs[x]
        y = regs[y]
...
def BFS(vertex):
    ...
    visit = [False] * num
    deque_ = deque()
    deque_.append(vertex)
    visit[vertex] = True
    paths[vertex][vertex] = 0
    number = 1
    path_ = 0
    while len(deque_) != 0:
        if number == 0:
...
for i in range(len(point) // 2):
    x = regions.nodes[point[2 * i].reg].get_set().num
    y = regions.nodes[point[2 * i + 1].reg].get_set().num
    print(paths[regs[x]][regs[y]])
